...
```bash
# 1. 克隆仓库
git clone https://github.com/tiantangyangyang-lang/worklife-map-china.git
cd worklife-map-china
...
> **Windows 贡献者注意**: 克隆后请执行一次 `git config core.fileMode false`。
> 否则 git 会把文件可执行位 (`100755`→`100644`) 当成改动, 导致 `git status`
> 满屏 "modified" 噪声 (并非真实内容变化)。
